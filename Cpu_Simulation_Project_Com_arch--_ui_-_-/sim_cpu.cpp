...
public:
    ...
    CPU() {
        registers[0] = 0; // Register 0 always 0 $zero
        initInstructionMap();
        initRegisterMap();
...
void CPU::execute(string instruction) {
    ...
    if(op == "lw " || op == "sw"){
        ...
        if (reg_map.find(rs_str) == reg_map.end()){
            throw runtime_error("Invalid source register: "+rs_str);            
        }
        int offset;
        try{
            offset = stoi(offset_str);
        }catch(...){
            throw runtime_error("Invalid offset valie: " + offset_str);
        }
...
int main() {
    ...
    cout << "t0: " << cpu.registers[8] << endl;  // output = 8  index reg 8 = $t
    cout << "s1: " << cpu.registers[17] << endl; // output = 3  index reg 17 = $s1
